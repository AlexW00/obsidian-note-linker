...
interface LinkTargetCandidatesListComponentProps {
	...
	setNoteChangeOperations: Dispatch<
		SetStateAction<Map<string, NoteChangeOperation>>
	>;
}
...
export const LinkTargetCandidatesListComponent = React.memo(
	({
		...
		const [selectedReplacement, setSelectedReplacement] =
			React.useState<Replacement>(_selectedReplacement);
		const createNoteChangeOperation = () => {
			return new NoteChangeOperation(parentNote.path, parentNote.content, [
				selectedReplacement,
			]);
		};

		const removeReplacement = () => {
			if (noteChangeOperation) {
				noteChangeOperation.replacements =
					noteChangeOperation.replacements?.filter(
		...
		const saveNoteChangeOperation = (
			...
			if (_noteChangeOperation.replacements.length == 0)
				_noteChangeOperations.delete(_noteChangeOperation.path);
		...
		return (
			<div className={"link-target-candidates-list"}>
				<LinkMatchTitleComponent
					matchedText={linkMatch.matchedText}
					position={linkMatch.position}
				/>
				<ul className={"hide-list-styling"}>
					{linkMatch.linkTargetCandidates.map(
						(linkTargetCandidate: LinkTargetCandidate) => (
							<LinkTargetCandidateContext.Provider
								value={linkTargetCandidate}
